...
def add_to_known_words():
    """Takes current selection, adds to text file, and clears its furigana."""
    ctx = uno.getComponentContext()
    smgr = ctx.ServiceManager
    doc = smgr.createInstanceWithContext("com.sun.star.frame.Desktop", ctx).getCurrentComponent()
    selection = doc.getCurrentSelection()
    ...
    try:
        ...
        # Clear furigana from the selection immediately
        remove_furigana_selection()
        duration = time.perf_counter() - start_time
        
        msgbox(f"'{selected_text}' added to exclusion list.\nTotal Time: {duration:.3f}s", "Word Excluded")
    except Exception as e:
